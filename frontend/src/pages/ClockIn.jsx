...
export default function ClockIn() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-900 to-blue-800 flex flex-col">
      {/* Header */}
      <div className="bg-blue-950 text-white px-6 py-5 text-center">
        <h1 className="text-2xl font-bold">Clock In / Out</h1>
        <p className="text-blue-200 text-sm mt-1">{user?.first_name} {user?.last_name}</p>
        <p className="text-3xl font-mono font-bold mt-2">{time.toLocaleTimeString()}</p>
        <p className="text-blue-300 text-xs">{time.toLocaleDateString()}</p>
      </div>
